...
from sam3.model.dem_fusion import (
    CrossModalAlignHead,
    DecoupledGatedPromptFusion,
    DEMCrossAttentionFusion,
    DEMDecoderFusion,
    DEMEncoder,
    DEMPromptFusion,
    DEMSpatialPromptTokens,
    TextAdapter,
    VerificationHead,
    VisualPromptAdapter,
)
...
def _create_vl_backbone(
    ...
    enable_dem_encoder_fusion: bool = True,
    dem_in_channels: int = 4,
    dem_fuse_all_levels: bool = True,
    enable_dem_prompt_fusion: bool = False,
    dem_prompt_fusion_mode: str = "add",
    enable_dem_prompt_rgb: bool = False,
    enable_dem_decoder_fusion: bool = False,
    dem_num_stages: int = 3,
    enable_dem_v3_refine: bool = False,
    enable_dem_spatial_prompt: bool = False,
    dem_spatial_grid: int = 8,
    enable_text_adapter: bool = False,
    text_adapter_rank: int = 64,
    enable_decoupled_prompt_fusion: bool = False,
    enable_cross_modal_align: bool = False,
    enable_visual_prompt: bool = False,
    enable_verification: bool = False,
):
    """Create visual-language backbone."""
    dem_encoder = None
    dem_fusion = None
    dem_prompt_fusion = None
    dem_spatial_prompt = None
    # V7 text-prompt modules (independent of DEM presence).
    text_adapter = TextAdapter(dim=256, rank=text_adapter_rank) if enable_text_adapter else None
    decoupled_prompt_fusion = (
        DecoupledGatedPromptFusion(dim=256) if enable_decoupled_prompt_fusion else None
    )
    align_head = CrossModalAlignHead(dim=256) if enable_cross_modal_align else None
    visual_prompt_adapter = VisualPromptAdapter(dim=256) if enable_visual_prompt else None
    verification_head = VerificationHead(dim=256) if enable_verification else None
    if enable_dem:
        dem_encoder = DEMEncoder(
            in_channels=dem_in_channels,
            out_channels=256,
            num_stages=dem_num_stages,
            v3_refine=enable_dem_v3_refine,
        )
        if enable_dem_encoder_fusion:
            dem_fusion = DEMCrossAttentionFusion(
                dim=256,
                fuse_all_levels=dem_fuse_all_levels,
                v3_refine=enable_dem_v3_refine,
            )
        if enable_dem_prompt_fusion:
            dem_prompt_fusion = DEMPromptFusion(
                dim=256,
                mode=dem_prompt_fusion_mode,
                use_rgb_prompt=enable_dem_prompt_rgb,
            )
        if enable_dem_spatial_prompt:
            dem_spatial_prompt = DEMSpatialPromptTokens(
                dim=256,
                grid_size=dem_spatial_grid,
            )
    return SAM3VLBackbone(
        ...
        dem_fusion=dem_fusion,
        dem_prompt_fusion=dem_prompt_fusion,
        dem_spatial_prompt=dem_spatial_prompt,
        text_adapter=text_adapter,
        decoupled_prompt_fusion=decoupled_prompt_fusion,
        align_head=align_head,
        visual_prompt_adapter=visual_prompt_adapter,
        verification_head=verification_head,
        enable_dem_decoder_fusion=(enable_dem and enable_dem_decoder_fusion),
    )
...
def _create_segmentation_head(
    ...
    enable_dem_decoder_fusion: bool = False,
    enable_dem_v3_refine: bool = False,
    enable_deep_seg_head: bool = False,
    enable_ppm_seg_head: bool = False,
):
    ...
    dem_decoder_fusion = (
        DEMDecoderFusion(dim=256, dem_dim=256, v3_refine=enable_dem_v3_refine)
        if enable_dem_decoder_fusion
        else None
    )
    ...
    segmentation_head = UniversalSegmentationHead(
        ...
        dem_decoder_fusion=dem_decoder_fusion,
        deep_seg_head=enable_deep_seg_head,
        ppm_seg_head=enable_ppm_seg_head,
    )
...
def build_sam3_image_model(
    ...
    Args:
        ...
        enable_dem: Whether to enable DEM terrain feature fusion for image inference/training
        dem_in_channels: Number of DEM-derived input channels, e.g. DEM, slope, sin/cos aspect
        dem_fuse_all_levels: Whether to fuse DEM features into all retained FPN levels
        enable_dem_prompt_fusion: Whether to inject global DEM vectors into text prompt features
        dem_prompt_fusion_mode: Fusion style for DEM-aware text prompts, either "add" or "concat"
    ...
    backbone = _create_vl_backbone(
        ...
        enable_dem_encoder_fusion=enable_dem_encoder_fusion,
        dem_in_channels=dem_in_channels,
        dem_fuse_all_levels=dem_fuse_all_levels,
        enable_dem_prompt_fusion=enable_dem_prompt_fusion,
        dem_prompt_fusion_mode=dem_prompt_fusion_mode,
        enable_dem_prompt_rgb=enable_dem_prompt_rgb,
        enable_dem_decoder_fusion=enable_dem_decoder_fusion,
        dem_num_stages=dem_num_stages,
        enable_dem_v3_refine=enable_dem_v3_refine,
        enable_dem_spatial_prompt=enable_dem_spatial_prompt,
        dem_spatial_grid=dem_spatial_grid,
        enable_text_adapter=enable_text_adapter,
        text_adapter_rank=text_adapter_rank,
        enable_decoupled_prompt_fusion=enable_decoupled_prompt_fusion,
        enable_cross_modal_align=enable_cross_modal_align,
    ...
    segmentation_head = (
        _create_segmentation_head(
            ...
            enable_dem_decoder_fusion=(enable_dem and enable_dem_decoder_fusion),
            enable_dem_v3_refine=enable_dem_v3_refine,
            enable_deep_seg_head=enable_deep_seg_head,
            enable_ppm_seg_head=enable_ppm_seg_head,
        )
