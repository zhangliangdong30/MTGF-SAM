...
class Sam3Processor:
    ...
    def set_image(self, image, dem=None, state=None):
        """Sets the image and optional DEM tensor on which we want to do predictions."""
        if state is None:
            state = {}
        ...
        dem_tensor = self._prepare_dem(dem) if dem is not None else None

        state["original_height"] = height
        state["original_width"] = width
        state["backbone_out"] = self.model.backbone.forward_image(image, dem=dem_tensor)
    ...
    def _prepare_dem(self, dem):
        if isinstance(dem, np.ndarray):
            dem = torch.from_numpy(dem)
        elif not isinstance(dem, torch.Tensor):
            raise ValueError("DEM must be a torch.Tensor or numpy.ndarray")

        dem = dem.to(device=self.device, dtype=torch.float32)
        if dem.dim() == 2:
            dem = dem.unsqueeze(0).unsqueeze(0)
        elif dem.dim() == 3:
            if dem.shape[0] in (1, 3, 4, 5, 6):
                dem = dem.unsqueeze(0)
            else:
                dem = dem.permute(2, 0, 1).unsqueeze(0)
        elif dem.dim() != 4:
            raise ValueError(
                f"DEM tensor must be [C,H,W], [H,W,C], or [B,C,H,W], got {dem.shape}"
            )
