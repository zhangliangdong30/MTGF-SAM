...
class SAM3VLBackbone(nn.Module):
    ...
    def __init__(
        ...
        scalp=0,
        dem_encoder: Optional[DEMEncoder] = None,
        dem_fusion: Optional[DEMCrossAttentionFusion] = None,
        dem_prompt_fusion: Optional[DEMPromptFusion] = None,
        dem_spatial_prompt: Optional["DEMSpatialPromptTokens"] = None,
        text_adapter: Optional["TextAdapter"] = None,
        decoupled_prompt_fusion: Optional["DecoupledGatedPromptFusion"] = None,
        align_head: Optional["CrossModalAlignHead"] = None,
        visual_prompt_adapter: Optional["VisualPromptAdapter"] = None,
        verification_head: Optional["VerificationHead"] = None,
        ...
        self.dem_fusion = dem_fusion
        self.dem_prompt_fusion = dem_prompt_fusion
        self.dem_spatial_prompt = dem_spatial_prompt
        self.text_adapter = text_adapter
        self.decoupled_prompt_fusion = decoupled_prompt_fusion
        self.align_head = align_head
        self.visual_prompt_adapter = visual_prompt_adapter
        self.verification_head = verification_head
        # The actual DEMDecoderFusion module lives on UniversalSegmentationHead; the
        # backbone only stashes the high-res DEM map when the flag is set.
        self.dem_decoder_fusion_enabled = enable_dem_decoder_fusion
    ...
    def forward(
        ...
        """
        output = self.forward_image(samples, dem=dem)
        device = output["vision_features"].device
        output.update(self.forward_text(captions, input_boxes, additional_text, device))
        return output

    def forward_image(self, samples: torch.Tensor, dem: Optional[torch.Tensor] = None):
    ...
    def _forward_image_no_act_ckpt(self, samples, dem=None):
        ...
        # RGB-only global summary, computed BEFORE any DEM fusion so the
        # vector fed to DEMPromptFusion as an "image prompt" is uncontaminated
        # by DEM signal. Pooled from the deepest (most semantic) RGB feature.
        rgb_global = None
        if (
            self.dem_prompt_fusion is not None
            and getattr(self.dem_prompt_fusion, "use_rgb_prompt", False)
        ):
            rgb_global = F.adaptive_avg_pool2d(
                sam3_features[-1], output_size=1
            ).flatten(1)

        dem_features_for_decoder = None
        dem_global = None
        dem_spatial_feat = None
        # DEM is used only when a DEM encoder is present; a model without one
        # (RGB-only ablation) silently ignores any DEM tensor.
        if dem is not None and self.dem_encoder is not None:
            dem = dem.to(device=sam3_features[-1].device, dtype=sam3_features[-1].dtype)
            # `return_highres=True` returns the stride-2 DEMEncoder output before
            # any cross-attention alignment slicing, so that an encoder with >3
            # stages still hands a high-res feature to DEMDecoderFusion.
            dem_features, dem_global, dem_highres = self.dem_encoder(
                dem,
                [x.shape[-2:] for x in sam3_features],
                return_global=True,
                return_highres=True,
            )
            # Scheme 3: keep the deepest (most semantic) aligned DEM feature map so
            # `_encode_prompt` can pool it into a grid of spatial prompt tokens.
            if self.dem_spatial_prompt is not None:
                dem_spatial_feat = dem_features[-1]
            # Encoder-level cross-attention fusion is optional (ablatable): it is
            # applied only when a DEMCrossAttentionFusion module is present, so
            # the encoded DEM features can still feed prompt/decoder fusion alone.
            if self.dem_fusion is not None:
                sam3_features = self.dem_fusion(sam3_features, dem_features)
            if self.dem_decoder_fusion_enabled:
                dem_features_for_decoder = dem_highres
        ...
        if dem_global is not None:
            output["dem_global"] = dem_global
        if rgb_global is not None:
            output["rgb_global"] = rgb_global
        if dem_features_for_decoder is not None:
            output["dem_feat_hires"] = dem_features_for_decoder
        if dem_spatial_feat is not None:
            output["dem_spatial_feat"] = dem_spatial_feat
