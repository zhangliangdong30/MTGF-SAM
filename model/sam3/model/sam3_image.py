...
class Sam3Image(torch.nn.Module):
    ...
    def _get_img_feats(self, backbone_out, img_ids):
        ...
        dem_batch = backbone_out.get("dem_batch_all_stages", None)
        if img_ids.numel() > 1:
        ...
        dem = None
        if dem_batch is not None:
            dem = dem_batch[unique_ids].to(dtype=torch.float32, device=self.device)
        ...
        backbone_out = {
            **backbone_out,
            **self.backbone.forward_image(image, dem=dem),
    ...
    def _encode_prompt(
        ...
        # Scheme 1 (V7): lightweight LoRA-style text adapter reshapes the (visually
        # phrased) text prompt embedding for the landslide concept. Zero-init no-op.
        if encode_text and getattr(self.backbone, "text_adapter", None) is not None:
            txt_feats = self.backbone.text_adapter(txt_feats)
        if (
            encode_text
            and "dem_global" in backbone_out
            and getattr(self.backbone, "dem_prompt_fusion", None) is not None
        ):
            terrain_vectors = backbone_out["dem_global"][find_input.img_ids]
            # Optional RGB prompt — only present when the prompt fusion was built
            # with use_rgb_prompt=True; otherwise the call falls back to the
            # text+DEM behaviour from earlier versions.
            rgb_vectors = None
            if "rgb_global" in backbone_out and getattr(
                self.backbone.dem_prompt_fusion, "use_rgb_prompt", False
            ):
                rgb_vectors = backbone_out["rgb_global"][find_input.img_ids]
            txt_feats = self.backbone.dem_prompt_fusion(
                txt_feats, terrain_vectors, rgb_vectors=rgb_vectors
            )
        # Scheme 2 (V7): decoupled, gated fusion of DEM-terrain + RGB-image context
        # via separate per-modality cross-attention + gates. Zero-init no-op.
        if (
            encode_text
            and "dem_global" in backbone_out
            and getattr(self.backbone, "decoupled_prompt_fusion", None) is not None
        ):
            terrain_vectors = backbone_out["dem_global"][find_input.img_ids]
            rgb_vectors = (
                backbone_out["rgb_global"][find_input.img_ids]
                if "rgb_global" in backbone_out
                else None
            )
            txt_feats = self.backbone.decoupled_prompt_fusion(
                txt_feats, terrain_vectors, rgb_vectors=rgb_vectors
            )
        ...
        if visual_prompt_embed is None:
            ...
            encode_text
            and getattr(self.backbone, "visual_prompt_adapter", None) is not None
        ):
            # V8: reference-image exemplar vectors -> learnable, zero-gated visual
            # prompt tokens appended to the prompt sequence.
            visual_prompt_embed = self.backbone.visual_prompt_adapter(
                visual_prompt_embed.to(geo_feats.dtype)
            )

        # Scheme 3: spatial DEM prompt tokens. When a DEMSpatialPromptTokens module
        # is present and the backbone stashed a DEM feature map, pool it into a grid
        # of location-aware terrain tokens and append them to the prompt sequence
        # (alongside the text + geometric tokens). Zero-gated at init → no-op until
        # learned. Only added on the text path, matching DEMPromptFusion.
        dem_spatial_tokens = None
        if (
            encode_text
            and "dem_spatial_feat" in backbone_out
            and getattr(self.backbone, "dem_spatial_prompt", None) is not None
        ):
            dem_feat = backbone_out["dem_spatial_feat"][find_input.img_ids]
            dem_spatial_tokens = self.backbone.dem_spatial_prompt(dem_feat)
            dem_spatial_tokens = dem_spatial_tokens.to(
                device=geo_feats.device, dtype=geo_feats.dtype
            )

        if encode_text:
            parts = [txt_feats, geo_feats]
            mask_parts = [txt_masks, geo_masks]
        else:
            parts = [geo_feats]
            mask_parts = [geo_masks]
        if dem_spatial_tokens is not None:
            n_tok, bsz = dem_spatial_tokens.shape[0], dem_spatial_tokens.shape[1]
            parts.append(dem_spatial_tokens)
            mask_parts.append(
                torch.zeros(
                    bsz, n_tok, device=geo_masks.device, dtype=geo_masks.dtype
                )
            )
        parts.append(visual_prompt_embed)
        mask_parts.append(visual_prompt_mask)
        prompt = torch.cat(parts, dim=0)
        prompt_mask = torch.cat(mask_parts, dim=1)
    ...
    def _run_segmentation_heads(
        ...
        if self.segmentation_head is not None:
            ...
            seg_head_outputs = activation_ckpt_wrapper(self.segmentation_head)(
                ...
                prompt=prompt,
                prompt_mask=prompt_mask,
                # New: high-res DEM feature for the optional pixel-decoder fusion.
                # Defaults to None when DEM input was not provided.
                dem_feat=backbone_out.get("dem_feat_hires", None),
            )
        ...
        **kwargs,
        ...
        with torch.profiler.record_function("SAM3Image._encode_prompt"):
            prompt, prompt_mask, backbone_out = self._encode_prompt(
                ...
                visual_prompt_embed=kwargs.get("visual_prompt_embed"),
    ...
    def forward(self, input: BatchedDatapoint):
        ...
        dem_batch = getattr(input, "dem_batch", None)
        if dem_batch is not None:
            backbone_out["dem_batch_all_stages"] = dem_batch
        backbone_out.update(self.backbone.forward_image(input.img_batch, dem=dem_batch))
